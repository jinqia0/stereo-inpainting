...
class GroupRandomHorizontalFlowFlipStereo(object):
    ...
    def __call__(self, frames, warpped_frames, masks, flows_f=None, flows_b=None):
        v = random.random()
        if v < 0.5:
            # Flip frames, warpped_frames, and masks
            ret_frames = [frame.transpose(Image.FLIP_LEFT_RIGHT) for frame in frames]
            ret_warpped_frames = [frame.transpose(Image.FLIP_LEFT_RIGHT) for frame in warpped_frames]
            ...
            if flows_f is not None and flows_b is not None:
                ...
                return ret_frames, ret_warpped_frames, ret_masks, ret_flows_f, ret_flows_b
            else:
                return ret_frames, ret_warpped_frames, ret_masks
        else:
            # No flip
            if flows_f is not None and flows_b is not None:
                return frames, warpped_frames, masks, flows_f, flows_b
            else:
                return frames, warpped_frames, masks


class GroupRandomHorizontalFlipStereo(object):
    """Randomly horizontally flips the given PIL.Images with a probability of 0.5."""
    
    def __call__(self, frames, warpped_frames, masks):
        v = random.random()
        if v < 0.5:
            # Flip frames, warpped_frames, and masks
            ret_frames = [frame.transpose(Image.FLIP_LEFT_RIGHT) for frame in frames]
